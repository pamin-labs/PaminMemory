...
// ---------------------------------------------------------------------------
// Scratch: what the two locks on the search path cost under concurrency.
//
// Appended by conc-harness.sh and reverted afterwards -- this is a measurement,
// not an assertion, so it follows joincost.rs rather than hubcost.rs and does
// not live in the repository. It is appended to this file rather than written
// as its own test because the warm index is keyed by the project UUID, and the
// project name is `xquad-{profile}-{corpus fingerprint}` -- reusing Corpus and
// profile() here is what guarantees it opens the index that already exists
// instead of building an empty one.
//
// A search takes two `std::sync::Mutex` guards in sequence: the embedder
// (engine.rs:496) for the forward pass, then the index (engine.rs:424) for the
// three recalls. Two arms separate them without touching the source:
//
//   (a) every request a query never asked before -- pays the forward pass, so
//       it meets both locks.
//   (b) every request from a set small enough to stay in the query cache --
//       the forward pass never runs, so whatever serialisation is left is the
//       index lock.
//
// (b) alone is the index lock's curve; (a) minus (b) is the embedder lock's.
// ---------------------------------------------------------------------------

/// Requests per configuration. Divisible by every concurrency below.
const REQUESTS: usize = 240;
const CONCURRENCY: [usize; 4] = [1, 2, 4, 8];
/// Kept under `Embedder::REMEMBERED_QUERIES` (256) so arm (b) never evicts.
...
impl Timing {
    fn throughput(&self) -> f64 {
        REQUESTS as f64 / (self.wall / 1000.0)
    }
...
/// Runs `REQUESTS` searches across `concurrency` tasks and times each one.
async fn drive(engine: &Engine, texts: Vec<String>, concurrency: usize) -> Timing {
    let engine = std::sync::Arc::new(engine.clone());
    let texts = std::sync::Arc::new(texts);
    let each = REQUESTS / concurrency;
    ...
    for worker in 0..concurrency {
        ...
        workers.push(tokio::spawn(async move {
            ...
            for request in 0..each {
                let text = &texts[(worker * each + request) % texts.len()];
                let one = std::time::Instant::now();
                let hits = engine
                    .search_fused(text, DEPTH as u32, DEPTHS, Fusion::default())
                    .await
                    .expect("search");
                latencies.push(one.elapsed().as_secs_f64() * 1000.0);
                assert!(!hits.is_empty(), "the warm index returned nothing");
            }
            latencies
        }));
    }
    ...
    for worker in workers {
        latencies.extend(worker.await.expect("a worker panicked"));
    }
    Timing {
...
#[ignore = "measurement: needs the warm PAMIN_EVAL_HOME index"]
async fn the_concurrency_ceiling() {
    let corpus = Corpus::load();
    let queries = corpus.queries();
    let (named, profile) = profile();

    let home = std::env::var("PAMIN_EVAL_HOME").expect("this needs the warm workspace");
    let workspace = Workspace::at(&home);
    let project = format!("xquad-{named}-{}", corpus.fingerprint());

    // `Connections::Resident`, not `Engine::open`'s `PerCommand`. PerCommand
    // caps the pool at four, and a search takes several connections, so eight
    // concurrent searches would queue on connections and the whole measurement
    // would be about the pool rather than about either lock. Resident is also
    // what `pamin serve` uses, which is the configuration the numbers are
    // supposed to describe.
    let database = pamin_store::Database::open(&workspace, pamin_store::Connections::Resident)
        .await
        .expect("open the database");
    let models = pamin_engine::Models::in_workspace(&workspace);
    let engine = Engine::attached(
        database, &models, &workspace, &project, profile, Access::ReadWrite,
    )
    .await
    .expect("open the engine");

    let threads = std::env::var("PAMIN_INFERENCE_THREADS").unwrap_or_else(|_| "all".into());
    println!("  {} queries, profile {named}, intra_threads {threads}", queries.len());

    // The cached side of every mix, warmed once so those requests are hits.
    let cached: Vec<String> = queries
        .iter()
        .take(CACHED)
    ...
    for text in &cached {
        engine
            .search_fused(text, DEPTH as u32, DEPTHS, Fusion::default())
            .await
            .expect("warm the query cache");
    }

    println!("\n  hit%    N   throughput q/s      p50 ms      p90 ms");
    println!("  ------------------------------------------------------");
    for hit_rate in [0usize, 50, 100] {
        for concurrency in CONCURRENCY {
            // One list per configuration, mixed to the requested hit rate: the
            // first slice comes from the warmed set and the rest carry a suffix
            // nothing has asked before, so they always pay a forward pass.
            let texts: Vec<String> = (0..REQUESTS)
                .map(|request| {
                    if request * 100 / REQUESTS < hit_rate {
                        cached[request % cached.len()].clone()
                    } else {
                        let base = queries[request % queries.len()].text();
                        format!("{base} {hit_rate}-{concurrency}-{request}")
                    }
                })
                .collect();
            let timing = drive(&engine, texts, concurrency).await;
            println!(
                "  {hit_rate:>4}  {concurrency:>3}   {:>14.1}   {:>9.1}   {:>9.1}",
                timing.throughput(),
                timing.at(50),
                timing.at(90)
            );
...
/// The core-count ceiling, with no lock of ours in the way.
///
/// `concurrency` separate `Embedder`s, each with its own model, so nothing is
/// shared and nothing is contended. Whatever this curve does is what four cores
/// can do, and the arms above cannot beat it however their locks are arranged.
/// Speed profile only: eight copies of BGE-M3 is several gigabytes, and this is
/// a reference line rather than a result about the shipping profile.
///
/// Every model is loaded, and every worker does one warm-up pass, before the
/// clock starts. A first version timed the loads too and reported 596 ms a pass
/// for a model that takes ten -- the load was the measurement.
#[tokio::test(flavor = "multi_thread", worker_threads = 8)]
#[ignore = "measurement: loads one model per concurrent worker"]
async fn what_four_cores_can_do() {
    let home = std::env::var("PAMIN_EVAL_HOME").expect("this needs the model cache");
    let cache = std::path::Path::new(&home).join("models");
    const PASSES: usize = 128;

    println!("\n       N   embeddings/s   per pass ms");
    println!("  -------------------------------------");
    for concurrency in CONCURRENCY {
        // Loaded and warmed first; the barrier is what keeps a slow loader from
        // being counted as a slow pass.
        let ready = std::sync::Arc::new(std::sync::Barrier::new(concurrency + 1));
        let each = PASSES / concurrency;
        let mut workers = Vec::new();
        for worker in 0..concurrency {
            ...
            workers.push(std::thread::spawn(move || {
                let mut embedder =
                    pamin_index::Embedder::load(Profile::Speed, &cache).expect("load the model");
                embedder.embed_query("warm up").expect("warm up");
                ready.wait();
                for pass in 0..each {
                    // A distinct text every time, so the query cache never
                    // answers and every pass is a real forward pass.
                    embedder
                        .embed_query(&format!("worker {worker} pass {pass}"))
                        .expect("embed");
                }
            }));
        }
        ...
        let done = (each * concurrency) as f64;
        println!(
            "  {concurrency:>6}   {:>12.1}   {:>11.1}",
            done / wall,
            wall * 1000.0 / done
        );
    }
