...
mod tests {
    ...
    /// Held by every test that changes the open-file limit.
    ///
    /// The limit is one per process and the test harness runs tests on
    /// threads, so two tests that each set it and then read it back race: one
    /// lowers it to 512, the other raises it to the ceiling in between, and
    /// the first reads back a number it never set. That failed CI once on a
    /// change that touched nothing here.
    ...
    ///
    /// The limit is process-wide, so it is put back, and taken under
    /// [`LIMIT`] so the other test here cannot move it in between.
    ///
