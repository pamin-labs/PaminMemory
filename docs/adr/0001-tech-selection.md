...
### What else zvec-rust 0.7.2 offers, measured: none of it ships
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| **fp32 HNSW (ships)** | **1.0000** | **0.9980** | **12.3 ms (5.95–12.55)** | **595.5 MB** | **565 MB** | — | **1** |
...
**The document iterator reads faster and saves nothing a rebuild can see.** A rebuild lends its vectors by reading the set-aside index twice through keyed fetches of 256 — once to count what it can lend, once to take it. Over MIRACL's 131,924 documents, warm, one pass of fetches took 1.46 to 1.59 s and one pass of the iterator 0.28 s, the resident set after each was the same 1.53 GB — the index, mapped — and the two read identical vectors and text. So the iterator would take about 2.5 s off a rebuild of that size; XQuAD-R's rebuild takes 30.0 s, of which reads at that rate are about 0.3 s. The rule asked for 10% of the rebuild's time or its peak memory, and the best case is 1%. It would also have to be kept off a writable collection — the iterator seals the segment being written each time it is created, and an open one blocks `optimize` — which the set-aside index is not, but the constraint would have been one more thing to hold.

The runs are scratch builds (a storage label read at open and the two caches bypassed) and cannot be re-run from the repository; `PAMIN_VECTOR_STORAGE` with `recall.rs` reproduces the synthetic int8 recall. The fp16 field needed about a hundred lines around the C API that the tree does not carry, since it does not ship.
