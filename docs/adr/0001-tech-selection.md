...
**What this leaves open, and how to settle it.** `reindex` calls `optimize` outright, so any corpus built by `import` followed by `reindex` has a graph — but a workspace written into by `pamin write` did not. Every published latency, throughput and resident-memory figure therefore has to be checked against how its corpus was built before it can be quoted again, and the answer is per figure rather than per page. The assertion is now in the tree (`crates/pamin-engine/tests/graphupkeep.rs`), and it fails on the code that preceded it, which was verified by reverting the condition and re-running rather than by argument.
